...
F_lo = np.zeros((nkpts,nval+nvirt,nval+nvirt), dtype=complex)
...
for ik in range(nkpts):
    ...
    F_lo[ik] = np.dot(np.dot(C_ao_lo[ik].T.conj(), F_ao[ik]), C_ao_lo[ik])

if use_pbe:
    fn = datadir + '/ks_lo_' + label + '.h5'
else:
    fn = datadir + '/hf_lo_' + label + '.h5'
