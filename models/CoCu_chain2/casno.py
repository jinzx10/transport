...
def make_casno_mp(mp, thresh=1e-4, nvir_act=None, nocc_act=None, vno_only=True,
...
def make_casno_gw(gw, thresh=1e-4, nvir_act=None, nocc_act=None, vno_only=True,
...
def make_casno_cc(mycc, thresh=1e-4, nvir_act=None, nocc_act=None, vno_only=True,
                  ea_cut=None, ip_cut=None, ea_no=None, ip_no=None, return_dm=False,
                  qp_cutoff=0.1, get_cas_mo=True, local=False, save_fcidump=False):
    '''
...
def make_casno_ucc(mycc, thresh=1e-4, nvir_act_a=None, nocc_act_a=None, nvir_act_b=None,
                   nocc_act_b=None, vno_only=True, return_dm=False, get_cas_mo=True):
    '''
...
def make_casno_cisd(myci, thresh=1e-4, nvir_act=None, nocc_act=None, vno_only=True,
    ...
    # NOTE: for Kondo non-SCF KS determinant, use saved dm and JK
    dm_hf = mf.make_rdm1()
    nimp = mf.nimp
    dm_hf[:nimp,:nimp] = mf.dm_init[:nimp,:nimp]
    ovlp = mf.get_ovlp()
    CS = np.dot(no_coeff.T, ovlp)
    dm_cas_no = np.dot(CS, np.dot(dm_hf, CS.T))
    JK_cas_no = _get_veff(dm_cas_no, g2e)[0]

    JK_full_hf_no = np.dot(no_coeff.T, np.dot(mf.JK_hf, no_coeff))
    JK_full_dft_no = np.dot(no_coeff.T, np.dot(mf.JK, no_coeff))
    h1e = h1e + JK_full_hf_no - JK_cas_no
    h1e = 0.5 * (h1e + h1e.T)
    comm.Barrier()

    dir='/home/zuxin/projects/transport/models/CoCu_chain/'
    h1e = comm.bcast(h1e, root=0)
    if rank == 0:
        fn = dir+'cas_g2e.h5'
        feri = h5py.File(fn, 'a')
    ...
    mf_cas.get_ovlp = lambda *args: np.eye(n_no)
    mf_cas._eri = g2e
    mf_cas.JK = JK_cas_no + (JK_full_dft_no - JK_full_hf_no)
    mf_cas.JK_hf = JK_cas_no

    Fcas = h1e + JK_cas_no + (JK_full_dft_no - JK_full_hf_no)
    if rank == 0:
        emo_ib, cmo_ib = np.linalg.eigh(Fcas)
        logger.info(mf, 'CAS mean-field mo_energy = \n %s', emo_ib)
        mo_occ_ib = np.zeros_like(emo_ib)
        mo_occ_ib[:nelectron//2] = 2.
    ...
    if get_cas_mo:
        ...
        if rank == 0:
            mf_cas.mo_coeff = cmo_ib
            mf_cas.mo_energy = emo_ib
            mf_cas.mo_occ = mo_occ_ib
        comm.Barrier()
        mf_cas.mo_occ = comm.bcast(mf_cas.mo_occ, root=0)
        mf_cas.mo_energy = comm.bcast(mf_cas.mo_energy, root=0)
        mf_cas.mo_coeff = comm.bcast(mf_cas.mo_coeff, root=0)
    ...
    if get_cas_mo:
        ...
        if return_dm:
            return mf_cas, no_coeff, h1e + JK_cas_no + (JK_full_dft_no - JK_full_hf_no), dm_cas_no, dm_gs
        else:
            return mf_cas, no_coeff, h1e + JK_cas_no + (JK_full_dft_no - JK_full_hf_no), dm_cas_no
