...
def kernel(dmft, mu, wl=None, wh=None, occupancy=None, delta=None,
    ...
    while not dmft_conv and cycle < max(1, dmft.max_cycle):
        ...
        if run_hf_embed:
            ...
            if dmft.mf_type == 'dft':
                # JK_dft_cell is the imp block's DFT JK potential from DFT-converged DM
                # which contains both imp-to-imp and bath-to-imp

                # JK_cell is the imp block's HF JK potential from DFT-converged DM
                # which also contains both imp-to-imp and bath-to-imp

                # JK_00 is intra-imp HF with DFT-converged DM

                # JK_cell-JK_00 is then the imp-bath part of the HF JK potential with DFT-converged DM

                # JK_dft_tmp = JK_dft_cell - (JK_cell-JK_00) is imp-to-imp DFT JK potential plus
                # the difference between DFT&HF's bath-to-imp potential (with DFT-converged DM)
