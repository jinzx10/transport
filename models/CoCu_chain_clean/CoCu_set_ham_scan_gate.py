...
# TODO scf probably needs to fix the chemical potential when using a gate voltage to shift Co's hcore
...
if os.path.isfile(mf_fname):
    ...
    # modify hcore to simulate gate voltage
    hcore_shifted = kmf.get_hcore()
    ovlp = kmf.get_ovlp()

    hcore_shifted[0:nao_Co_tot, 0:nao_Co_tot] += gate * ovlp[0:nao_Co_tot, 0:nao_Co_tot]
    kmf.get_hcore = lambda *args : hcore_shifted
...
fname = datadir + '/C_ao_lo_' + cell_label + '_' + method_label + '_' + gate_label + '.h5'
f = h5py.File(fname, 'w')
f['C_ao_lo'] = C_ao_lo
...
fn = datadir + '/DM_lo_' + cell_label + '_' + method_label + '_' + gate_label + '.h5'
f = h5py.File(fn, 'w')
f['DM_lo'] = DM_lo
f['DM_lo_Co'] = DM_lo_Co
f.close()
...
fn = datadir + '/eri_lo_' + cell_label + '_' + method_label + '_' + gate_label + '.h5'
f = h5py.File(fn, 'w')
...
fn = datadir + '/hcore_JK_lo_' + cell_label + '_' + method_label + '_' + gate_label + '.h5'
